...
hstadel$Sample <- "HST"
spy$Sample <- "Spy"
lescottes$Sample <- "lesCottes"

depth_all <- bind_rows(scladina, denny, mez1, hstadel, spy, lesCottes)

depth_all <- depth_all %>%
  mutate(
    Chromosome = factor(Chromosome, levels = unique(Chromosome)),
    Sample = factor(Sample, levels = c("Scladina", "Denny", "Mez1", "HST", "Spy", "lesCottes"))
  )
```
