...
```{r depth, include = FALSE, message = FALSE, warning = FALSE}
...
depth_all <- bind_rows(scladina, denny, mez1, mez2, hstadel, spy, lescottes, den5, chag8, vindija87)

depth_all <- depth_all %>%
  mutate(
    Chromosome = factor(Chromosome, levels = unique(Chromosome)),
    Sample = factor(Sample, levels = c("Scladina", "Denisova 11 (Denny)", "Mez1", "Mez2", "HST", "Spy", "les Cottes", "Denisova 5 (Altai Neanderthal)", "Chagyrskaya 8", "Vindija 87"))
  )
```

```{r depth_viz}
wes_samples <- wes_palette("Zissou1", 10, type = "continuous")
