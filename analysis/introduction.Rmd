...
snps_plot <- snps_set %>%
  mutate(
    Chromosome = sub("\\:.*", "", locations)
  ) %>%
  separate_rows(mappedGenes, sep = ",") %>%
  mutate(Gene = str_trim(mappedGenes)) %>%
  mutate(
    Category = ifelse(Gene %in% target_genes, Gene, "Other SNPs")
  ) %>%
  drop_na(Chromosome)

snps_plot <- snps_plot %>%
  count(Chromosome, Category)
...
snps_plot <- snps_plot %>%
  mutate(
    ...
    Category = factor(
      Category,
      levels = c("Other SNPs", target_genes)
    )
  )

wes_cols <-  wes_palette("Zissou1", 10, type = "continuous")
