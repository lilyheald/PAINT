...
```{r chr5, echo = FALSE, message = FALSE, warning = FALSE}
chr5_genes <- snps_set %>%
  mutate(Chromosome = sub("\\:.*", "", locations)) %>%
  filter(Chromosome == "5") %>%
  separate_rows(mappedGenes, sep = ",") %>%
...
chr5_summary <- chr5_genes %>%
  count(Gene, sort = TRUE)

head(chr5_summary, 10)

library(ggplot2)

top_chr5 <- chr5_summary %>%
  slice_max(n, n = 10)

ggplot(top_chr5, aes(x = reorder(Gene, n), y = n)) +
  geom_col(fill = "grey40", width = 0.7) +
  coord_flip() +
  labs(
    x = "Gene",
    y = "Number of SNPs",
    title = "Top genes on chromosome 5"
  ) +
