...
depth_all <- bind_rows(scladina, denny, mez1, hstadel, spy, lescottes, den5)

depth_all <- depth_all %>%
  mutate(
    Chromosome = factor(Chromosome, levels = unique(Chromosome)),
    Sample = factor(Sample, levels = c("Scladina", "Denny", "Mez1", "HST", "Spy", "les Cottes", "Altai Neanderthal"))
  )
```

```{r depth_viz}
wes_samples <- wes_palette("Zissou1", 8, type = "continuous")
