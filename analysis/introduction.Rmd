...
depth_all <- bind_rows(scladina, denny, mez1, hstadel)

depth_all <- depth_all %>%
  mutate(
    Chromosome = factor(Chromosome, levels = unique(Chromosome)),
    Sample = factor(Sample, levels = c("Scladina", "Denny", "Mez1", "HST"))
  )

wes_samples <- wes_palette("Darjeeling1", 4, type = "discrete")
